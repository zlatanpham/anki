 ...
 */
export function validateClozeText(clozeText: string): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  const clozeRegex = /\{\{c(\d+)::([^}]+)\}\}/g;
  const matches = [...clozeText.matchAll(clozeRegex)];
  ...
  if (allMatches.length !== matches.length) {
    errors.push("Some cloze deletions have malformed syntax. Use {{c1::text}} format.");
  }
 ...
 */
export function previewClozeCards(clozeText: string): Array<{ cardNumber: number; question: string; answer: string }> {
