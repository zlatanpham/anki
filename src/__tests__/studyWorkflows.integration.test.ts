...
describe("Study Workflow Integration Tests", () => {
  describe("Complete study session workflow", () => {
    ...
    test("should handle different rating patterns over multiple sessions", () => {
      let cardState = SuperMemo2Algorithm.scheduleNewCard();
      const ratingPattern: ReviewRating[] = [
        "GOOD",
        "GOOD",
        "HARD",
        "GOOD",
  ...
  describe("Due date calculations and scheduling", () => {
    test("should correctly calculate due dates across different time zones", () => {
      ...
      const result = SuperMemo2Algorithm.calculateNextReviewResult(
        "GOOD",
        cardState,
      );
      ...
      expect(
        SuperMemo2Algorithm.daysUntilDue(
    ...
    test("should handle overdue cards correctly", () => {
      ...
      expect(SuperMemo2Algorithm.isCardDue(cardState)).toBe(true);
      expect(SuperMemo2Algorithm.daysUntilDue(cardState)).toBeLessThanOrEqual(
        0,
      );
    ...
    test("should provide accurate card state descriptions", () => {
      // New card
      let cardState = SuperMemo2Algorithm.scheduleNewCard();
      expect(SuperMemo2Algorithm.getCardStateDescription(cardState)).toBe(
        "New",
      );

      // Learning card
      cardState = SuperMemo2Algorithm.calculateNextReview("AGAIN", cardState);
      expect(SuperMemo2Algorithm.getCardStateDescription(cardState)).toContain(
        "Learning",
      );
  ...
  describe("Batch study session simulation", () => {
    ...
    test("should maintain consistent statistics across session", () => {
      ...
      const sessionRatings: ReviewRating[] = [
        "GOOD",
        "GOOD",
        "HARD",
        "GOOD",
      ...
      sessionRatings.forEach((rating) => {
        currentState = SuperMemo2Algorithm.calculateNextReview(
          rating,
          currentState,
        );
  ...
  describe("Edge cases in study workflows", () => {
    ...
    test("should handle alternating performance patterns", () => {
      ...
      const alternatingPattern: ReviewRating[] = [
        "GOOD",
        "HARD",
        "GOOD",
        "HARD",
    ...
    test("should handle extreme easiness factor scenarios", () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (lowEFCard.state === "LEARNING") {
          lowEFCard = SuperMemo2Algorithm.calculateNextReview(
            "GOOD",
            lowEFCard,
          );
        }
      ...
      for (let i = 0; i < 5; i++) {
        highEFCard = SuperMemo2Algorithm.calculateNextReview(
          "EASY",
          highEFCard,
        );
      }
  ...
  describe("Real-world usage patterns", () => {
    ...
    test("should handle mixed deck study session", () => {
      ...
      const reviewResults = deckCards.map((card, index) => {
        ...
        return rating
          ? SuperMemo2Algorithm.calculateNextReview(rating, card)
          : card;
      });

      // Analyze session results
      const newToReview = reviewResults.filter(
        (result, index) =>
          deckCards[index]!.state === "NEW" && result.state === "REVIEW",
      ).length;

      const totalLapses = reviewResults.reduce(
        (sum, result) => sum + result.lapses,
        0,
      );
      ...
      expect(
        reviewResults.every((result) => result.dueDate instanceof Date),
      ).toBe(true);
    });
  });
});
