...
async function main() {
  ...
  const mathCards = [
    ...
    },
    {
      ...
      cloze_text: '<p>In a right triangle: {{c1::a²}} + {{c2::b²}} = {{c3::c²}}</p>',
  ...
  const historyCards = [
    ...
    },
    {
      ...
      cloze_text: '<p>The United States declared independence in {{c1::1776}} from {{c2::Great Britain}}.</p>',
    ...
    if (reviewData && card) {
      ...
      await prisma.review.create({
        data: {
          ...
          previous_interval: 0,
          new_interval: reviewData.rating === 'AGAIN' ? 0 : reviewData.rating === 'HARD' ? 1 : 4,
          easiness_factor: 2.5,
        },
      });
    }
