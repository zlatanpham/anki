...
describe("SuperMemo2Algorithm", () => {
  describe("New card scheduling", () => {
    ...
    test("should set due date to now for new cards", () => {
      ...
      expect(newCardState.dueDate.getTime()).toBeGreaterThanOrEqual(
        before.getTime(),
      );
      expect(newCardState.dueDate.getTime()).toBeLessThanOrEqual(
        after.getTime(),
      );
    });
  });
  ...
  describe("Rating progression", () => {
    test("AGAIN rating should reset card to learning", () => {
      ...
      const result = SuperMemo2Algorithm.calculateNextReview(
        "AGAIN",
        cardState,
      );
  ...
  describe("Due date calculation", () => {
    test("should set due date based on interval", () => {
      ...
      const timeDiff = Math.abs(
        result.dueDate.getTime() - expectedDueDate.getTime(),
      );
